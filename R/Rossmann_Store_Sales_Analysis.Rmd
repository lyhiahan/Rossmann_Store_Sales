...
## 5.1 Biểu đồ nâng cao & Phân tích Chiến lược

### 5.1.1 Phân phối Doanh số và Khách hàng
...
### 5.1.2 Kiểm định Phân phối chuẩn và Biến đổi Log-scale
...
### 5.1.3 Xu hướng Doanh số theo Thời gian (Time Series Trend)
...
### 5.1.4 Phân tích Biến động theo Tháng và Ngày trong tuần
...
### 5.1.5 Tác động của Ngày lễ Quốc gia và Kỳ nghỉ học

```{r holiday-impact, echo=TRUE, fig.cap="Hình 8: Ảnh hưởng ngày lễ quốc gia & kỳ nghỉ của học sinh"}
holiday_comp <- bind_rows(
  DuLieu %>% mutate(type = ifelse(state_holiday != "none", "Ngày lễ", "Ngày thường"),
                cat = "State Holiday") %>%
    group_by(cat, type) %>%
    summarise(mean_sales = mean(sales), .groups = "drop"),
  DuLieu %>% mutate(type = ifelse(school_holiday == 1, "Nghỉ học", "Ngày thường"),
                cat = "School Holiday") %>%
    group_by(cat, type) %>%
    summarise(mean_sales = mean(sales), .groups = "drop")
)

p_holiday <- ggplot(holiday_comp, aes(x = type, y = mean_sales, fill = type)) +
  geom_col(alpha = 0.85, width = 0.6) +
  geom_text(aes(label = scales::comma(round(mean_sales))), vjust = -0.6, size = 3.5, fontface = "bold") +
  facet_wrap(~cat, scales = "free_x") +
  scale_fill_manual(values = c("Ngày lễ"     = "#E53935",
                                "Nghỉ học"    = "#FF9800",
                                "Ngày thường" = "#2196F3")) +
...
### 5.1.6 Phân phối doanh số theo Assortment

```{r assortment-boxplot, echo=TRUE, fig.cap="Hình 10: Phân phối doanh số theo Assortment"}
p_assort <- ggplot(DuLieu, aes(x = assortment, y = sales, fill = assortment)) +
  geom_boxplot(alpha = 0.7, outlier.alpha = 0.1, outlier.size = 0.5) +
  scale_fill_manual(values = c("a" = "#2196F3", "b" = "#E53935", "c" = "#4CAF50"),
                    labels = c("a" = "Basic", "b" = "Extra", "c" = "Extended")) +
  scale_x_discrete(labels = c("a" = "Basic", "b" = "Extra", "c" = "Extended")) +
  scale_y_continuous(labels = scales::comma) +
  labs(
    title    = "Phân phối doanh số theo Assortment",
    x = "Cơ cấu sản phẩm", y = "Doanh số (EUR)"
  ) +
  GiaoDienRossmann + theme(legend.position = "none")
print(p_assort)
```

**Ứng dụng thực tế**: Cấp độ assortment **c** (Extended - mở rộng) mang lại doanh số trung bình cao nhất, cho thấy việc đa dạng hóa sản phẩm giúp kích thích chi tiêu của khách hàng.

### 5.1.7 Doanh thu trung bình trên mỗi khách hàng theo loại cửa hàng

```{r violin-spc, echo=TRUE, fig.cap="Hình 11: Doanh thu trung bình trên mỗi khách hàng theo loại cửa hàng"}
p_violin <- ggplot(DuLieu, aes(x = store_type, y = sales_per_customer, fill = store_type)) +
  geom_violin(alpha = 0.6, color = "white", draw_quantiles = c(0.25, 0.5, 0.75)) +
  scale_fill_manual(values = MauCuaHang) +
  scale_x_discrete(labels = c("a" = "Type A", "b" = "Type B", "c" = "Type C", "d" = "Type D")) +
  scale_y_continuous(labels = scales::comma) +
  labs(
    title    = "Doanh thu trên khách hàng theo Store Type",
    x = "Loại cửa hàng", y = "Doanh thu/Khách hàng (EUR)"
  ) +
  GiaoDienRossmann + theme(legend.position = "none")
print(p_violin)
```
...
### 5.1.8 Tác động của Khuyến mãi đến Doanh số theo Store Type
...
p_promo_bar <- ggplot(promo_impact, aes(x = store_type, y = mean_sales, fill = factor(promo))) +
  ...
  geom_text(aes(label = scales::comma(round(mean_sales))),
            position = position_dodge(0.8), vjust = -1, size = 3, fontface = "bold") +
  scale_fill_manual(values = c("0" = "#90A4AE", "1" = "#E53935"), labels = c("0" = "Không KM", "1" = "Có KM")) +
  scale_x_discrete(labels = c("a"="Type A","b"="Type B","c"="Type C","d"="Type D")) +
  scale_y_continuous(labels = scales::comma, expand = expansion(mult = c(0, 0.18))) +
  labs(
    title    = "Tác động khuyến mãi theo loại cửa hàng",
...
**Ứng dụng thực tế**:
- Khuyến mãi mang lại sự gia tăng doanh thu đồng loạt trên mọi cửa hàng.
- Cửa hàng loại **b** đạt mức tăng tuyệt đối lớn nhất khi có khuyến mãi, nhưng cửa hàng loại **a** và **d** cũng ghi nhận tỷ lệ tăng trưởng doanh thu rất ấn tượng khi áp dụng chương trình. Điều này gợi ý doanh nghiệp nên tối ưu hóa tần suất khuyến mãi tùy theo đặc trưng lưu lượng khách của từng khu vực.

### 5.1.9 Tỷ lệ Giao dịch Ngoại lai theo từng Cửa hàng
...
### 5.1.11 Ảnh hưởng thời gian hoạt động của đối thủ cạnh tranh đến Doanh số
...
### 5.1.12 Tỷ lệ Outlier theo Tháng
...
### 5.1.13 Hiệu quả Khuyến mãi theo Ngày trong tuần (Dumbbell Chart)
...
### 5.1.14 Hiệu ứng Cộng hưởng giữa Khuyến mãi Định kỳ và Khuyến mãi Dài hạn
...
### 5.1.15 Phân tích Tăng trưởng Tháng-so-với-Tháng (Waterfall Chart)
...
### 5.1.16 Phân tích Pareto — Top cửa hàng đóng góp 80% tổng doanh thu
...
### 5.1.17 Lollipop Chart — Xếp hạng Hiệu suất 50 Cửa hàng
...
### 5.1.18 Toàn cảnh Hiệu suất 50 Cửa hàng Rossmann — Biểu đồ Bong bóng Đa chiều
...
### 5.1.19 Ma trận Chiến lược BCG — Phân loại 50 Cửa hàng
